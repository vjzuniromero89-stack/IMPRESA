...
- `ensureBusiness()` ahora guarda el id del negocio en el navegador (localStorage) la primera vez que lo resuelve, y lo reutiliza siempre. Antes, si por cualquier motivo llegaba a haber más de una fila en la tabla `businesses` de Supabase (por ejemplo de pruebas de sesiones anteriores), cada carga de página podía terminar usando un negocio distinto — dando la impresión de que algo recién guardado (como una deuda) desaparecía al refrescar, cuando en realidad quedó guardado, pero bajo otro negocio.
