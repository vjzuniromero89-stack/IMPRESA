...
function MoneyControl({sales,expenses,accounts,month,rate,accountHistory}:any){
 ...
 const period=history.filter(h=>!isInitial(h)&&!isTransfer(h)&&hDate(h).startsWith(month)&&Math.abs(delta(h))>=0.005).sort((a,b)=>hDate(a).localeCompare(hDate(b))||String(a.at).localeCompare(String(b.at)));
...
function Accounts({accounts,setAccounts,rate,businessId,logCtx,accountHistory,reloadAccountHistory}:any){
 ...
 const generalRows=generalEntries.map(h=>{
  ...
  const deltaC=round(h.currency==='US$'?delta*usedRate:delta);
  ...
  return [movementDate(h),<div className="generalMovementDescription" key={`desc-${h.id}`}><strong>{movementDetail(h)}</strong><span>{accountName} · {h.currency}{h.currency==='US$'?` · TC ${usedRate.toFixed(4)}`:''}</span></div>,delta>0?money(delta,h.currency):'—',delta<0?money(Math.abs(delta),h.currency):'—',dual(endingC,rate)];
 });
