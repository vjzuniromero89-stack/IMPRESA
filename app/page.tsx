...
function Accounts({accounts,setAccounts,rate,businessId,logCtx,accountHistory,reloadAccountHistory}:any){
 ...
 const generalRows=generalEntries.map(h=>{const delta=movementDelta(h),endingC=runningC,endingUSD=runningUSD;const accountName=h.accountName||orderedAccounts.find(a=>a.id===h.accountId)?.name||'Cuenta';if(h.currency==='US$')runningUSD=round(runningUSD-delta);else runningC=round(runningC-delta);return [movementDate(h),<div className="generalMovementDescription" key={`desc-${h.id}`}><strong>{movementDetail(h)}</strong><span>{accountName} · {h.currency}</span></div>,delta>0?money(delta,h.currency):'—',delta<0?money(Math.abs(delta),h.currency):'—',<div className="mixedBalanceCell" key={`balance-${h.id}`}><strong>{money(endingC,'C$')}</strong><span>{money(endingUSD,'US$')}</span></div>]});
