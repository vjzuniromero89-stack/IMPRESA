...
function Accounts({accounts,setAccounts,rate,businessId,logCtx,accountHistory,reloadAccountHistory}:any){
 ...
 const recordBalance=async(a:Account,newBalance:number)=>{if(!businessId)return;try{await addAccountBalanceHistoryRemote(businessId,{id:uid(),accountId:a.id,accountName:a.name,currency:a.currency,previousBalance:a.balance,newBalance,changedBy:logCtx?.username})}catch(err){console.error('IMPRESA: historial de transferencia',err)}};
 ...
 {historyAccountId&&(()=>{const a=(accounts as Account[]).find(x=>x.id===historyAccountId);if(!a)return null;const entries=((accountHistory as AccountBalanceEntry[])||[]).filter(h=>h.accountId===a.id);return <Panel title={`Historial de saldo · ${a.name}`}>{!entries.length?<Empty text="Esta cuenta todavía no tiene cambios de saldo registrados."/>:<Table heads={['Fecha','Saldo anterior','Saldo nuevo','Usuario','Acción']} rows={entries.map(h=>[String(h.at).slice(0,16).replace('T',' '),money(h.previousBalance,h.currency),money(h.newBalance,h.currency),h.changedBy||'—',<button className="dangerSmall" onClick={()=>removeHistoryEntry(h)}>Borrar</button>])}/>}</Panel>})()}
