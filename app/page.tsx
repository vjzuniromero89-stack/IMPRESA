...
function MoneyControl({sales,expenses,accounts,month,rate,accountHistory}:any){
 ...
 for(const h of period){const d=delta(h),c=deltaC(h);running=round(running+c);const account=h.accountName||(accounts as Account[]).find(a=>a.id===h.accountId)?.name||'Cuenta';const amountLabel=h.currency==='US$'?`${money(Math.abs(d),'US$')} · ${money(Math.abs(c),'C$')}`:dual(Math.abs(c),rate);flowRows.push([hDate(h),d>0?'Entrada':'Salida',`${detail(h)} · ${account}`,d>0?amountLabel:'—',d<0?amountLabel:'—',dual(running,rate)])}
