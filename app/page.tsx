...
function Accounts({accounts,setAccounts,rate,businessId,logCtx,accountHistory,reloadAccountHistory}:any){
 ...
 const generalRows=generalEntries.map(h=>{
  ...
  return [movementDate(h),<div className="generalMovementDescription" key={`desc-${h.id}`}><strong>{movementDetail(h)}</strong><span>{accountName} · {h.currency}{h.currency==='US$'?` · TC ${usedRate.toFixed(4)}`:''}</span></div>,delta>0?money(delta,h.currency):'—',delta<0?money(Math.abs(delta),h.currency):'—',money(endingC,'C$')];
 });
