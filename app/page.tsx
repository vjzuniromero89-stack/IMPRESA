...
function Accounts({accounts,setAccounts,rate,businessId,logCtx,accountHistory,reloadAccountHistory}:any){
 ...
 const recordBalance=async(a:Account,newBalance:number,note='Transferencia')=>{if(!businessId)return;try{await addAccountBalanceHistoryRemote(businessId,{id:uid(),accountId:a.id,accountName:a.name,currency:a.currency,previousBalance:a.balance,newBalance,changedBy:`${logCtx?.username||'Sistema'} · ${note}`})}catch(err){console.error('IMPRESA: historial de transferencia',err)}};
 const update=async(a:Account)=>{const x=prompt(`Saldo actual de ${a.name} (${a.currency})`,String(a.balance));if(x===null||isNaN(+x))return;const newBalance=+x;if(newBalance===a.balance)return;if(businessId){try{await addAccountBalanceHistoryRemote(businessId,{id:uid(),accountId:a.id,accountName:a.name,currency:a.currency,previousBalance:a.balance,newBalance,changedBy:logCtx?.username});reloadAccountHistory&&reloadAccountHistory()}catch(err){console.error(err);alert('No se pudo guardar el historial de este cambio, pero el saldo sí se actualizará.')}}setAccounts(accounts.map((z:Account)=>z.id===a.id?{...z,balance:newBalance,updated:today()}:z))};
 const beginTransfer=(a:Account)=>{setTransferFrom(a);setTransferTo((accounts as Account[]).find(x=>x.id!==a.id)?.id||'');setTransferAmount('')};
 const doTransfer=async()=>{if(!transferFrom)return;const dest=(accounts as Account[]).find(a=>a.id===transferTo);const amount=Number(transferAmount);if(!dest||!Number.isFinite(amount)||amount<=0)return alert('Selecciona la cuenta destino y escribe un monto válido.');if(amount>transferFrom.balance)return alert(`Saldo insuficiente en ${transferFrom.name}. Disponible: ${money(transferFrom.balance,transferFrom.currency)}.`);if(rate<=0)return alert('El tipo de cambio debe ser mayor que cero.');let received=amount;if(transferFrom.currency!==dest.currency)received=transferFrom.currency==='C$'?amount/rate:amount*rate;const sourceNew=transferFrom.balance-amount,destNew=dest.balance+received;await Promise.all([recordBalance(transferFrom,sourceNew,`Transferencia enviada a ${dest.name}`),recordBalance(dest,destNew,`Transferencia recibida de ${transferFrom.name}`)]);await setAccounts((accounts as Account[]).map(a=>a.id===transferFrom.id?{...a,balance:sourceNew,updated:today()}:a.id===dest.id?{...a,balance:destNew,updated:today()}:a));reloadAccountHistory&&reloadAccountHistory();alert(`Transferencia realizada. Salieron ${money(amount,transferFrom.currency)} de ${transferFrom.name} y entraron ${money(received,dest.currency)} a ${dest.name}.`);setTransferFrom(null);setTransferAmount('')};
 const toggleHistory=(a:Account)=>setHistoryAccountId(historyAccountId===a.id?null:a.id);
 const removeHistoryEntry=async(h:AccountBalanceEntry)=>{if(!confirm(`¿Borrar esta línea del historial (${money(h.previousBalance,h.currency)} → ${money(h.newBalance,h.currency)})? Esto solo borra el registro, no cambia el saldo actual de la cuenta.`))return;try{await removeAccountBalanceHistoryRemote(h.id);reloadAccountHistory&&reloadAccountHistory()}catch(err){console.error(err);alert('No se pudo borrar en la nube.')}};
 ...
 <Panel title="Historial general de Banco y Efectivo">{!accountHistory?.length?<Empty text="Todavía no hay movimientos registrados en las cuentas."/>:<Table heads={['Fecha','Cuenta','Moneda','Saldo anterior','Saldo nuevo','Movimiento','Usuario','Acción']} rows={[...(accountHistory as AccountBalanceEntry[])].sort((a,b)=>String(b.at).localeCompare(String(a.at))).map(h=>{const delta=Number(h.newBalance)-Number(h.previousBalance);return [String(h.at).slice(0,16).replace('T',' '),h.accountName||((accounts as Account[]).find(a=>a.id===h.accountId)?.name||'Cuenta'),h.currency,money(h.previousBalance,h.currency),money(h.newBalance,h.currency),`${delta>=0?'+':''}${money(delta,h.currency)}`,h.changedBy||'—',<button className="dangerSmall" onClick={()=>removeHistoryEntry(h)}>Borrar</button>]})}/>}</Panel>
 {showNewAccount&&<div className="accountModalBackdrop" onMouseDown={()=>setShowNewAccount(false)}><div className="accountModal" onMouseDown={e=>e.stopPropagation()}><div className="accountModalHead"><div><span>NUEVA CUENTA</span><h3>Crear cuenta o caja</h3></div><button className="modalClose" onClick={()=>setShowNewAccount(false)}>×</button></div><div className="form grid"><Input l="Nombre de la cuenta" v={f.name} s={v=>setF({...f,name:v})}/><Select l="Moneda" v={f.currency} s={v=>setF({...f,currency:v})} opts={['C$','US$']}/></div><div className="note">La cuenta se crea con saldo C$0.00 o US$0.00. El saldo inicial se registra desde Configuración.</div><div className="actions modalActions"><button className="btn" onClick={()=>setShowNewAccount(false)}>Cancelar</button><button className="btn primary" onClick={add}>Crear cuenta</button></div></div></div>}</>}
...
function Settings({rate,setRate,initialBase,accounts,setAccounts,businessId,logCtx,reloadAccountHistory}:any){
 ...
 const saveInitialBalance=async(a:Account)=>{
  ...
  try{
   if(businessId)await addAccountBalanceHistoryRemote(businessId,{id:uid(),accountId:a.id,accountName:a.name,currency:a.currency,previousBalance:a.balance,newBalance:amount,changedBy:logCtx?.username});
