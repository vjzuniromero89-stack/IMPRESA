...
type SalesProps={sales:Sale[];setSales:Setter<Sale>;month:string;rate:number};
...
const methods:PaymentMethod[]=['Transferencia','Efectivo'];
function Field({label,children}:{label:string;children:ReactNode}){return <label><span>{label}</span>{children}</label>}
function Method({value,onChange,allowEmpty=false}:{value:string;onChange:(v:PaymentMethod)=>void;allowEmpty?:boolean}){return <Field label="Método de pago"><select required value={value} onChange={e=>onChange(e.target.value as PaymentMethod)}>{allowEmpty&&<option value="">Selecciona transferencia o efectivo</option>}{methods.map(m=><option key={m}>{m}</option>)}</select></Field>}
...
export function Sales({sales,setSales,month,rate}:SalesProps){
 ...
 <CurrencyField value={f.currency} onChange={currency=>setF({...f,currency})}/><Method value={f.paymentMethod} allowEmpty onChange={paymentMethod=>setF({...f,paymentMethod})}/>
...
export function SalesMethods({sales,month,rate}:SalesProps){
 const [all,setAll]=useState(false);const visible=sales.filter(s=>all||s.date.startsWith(month));
 const groups=[...methods,'Sin clasificar'] as const;
 ...
 {groups.map(method=>{const rows=visible.filter(s=>(s.paymentMethod||'Sin clasificar')===method).slice().sort((a,b)=>b.date.localeCompare(a.date));const collected=payments.filter(p=>(p.method||'Sin clasificar')===method);return <section className="panel historySection" key={method}><h2>{method} · {rows.length} ventas</h2>{method==='Sin clasificar'&&<p className="muted">Los registros antiguos conservan sus datos. Puedes asignar el método de la venta desde Editar en Ventas. Los abonos antiguos sin método permanecen sin clasificar.</p>}<GridTable heads={['Fecha de venta','Cliente','Trabajo','Total de venta','Cobrado (todos los métodos)','Saldo','Estado']}>{rows.map(s=><tr key={s.id}><td>{s.date}</td><td>{s.client}</td><td>{s.description||'—'}</td><td>{dual(s.amount,rate)}</td><td>{money(paid(s))}</td><td>{money(Math.max(0,s.amount-paid(s)))}</td><td><span className={'statusBadge '+(s.status==='Pagada'?'paid':s.status==='Pendiente'?'pending':'partial')}>{s.status}</span></td></tr>)}</GridTable>{!rows.length&&<p className="empty">No hay ventas en este grupo.</p>}<h2>Pagos recibidos · {method}</h2><GridTable heads={['Fecha del pago','Cliente','Importe','Concepto']}>{collected.map(p=><tr key={p.id}><td>{p.date}</td><td>{p.client}</td><td>{dual(p.amount,rate)}</td><td>{p.note||'Abono'}</td></tr>)}</GridTable>{!collected.length&&<p className="empty">No hay pagos detallados en este grupo.</p>}</section>})}</>;
}
