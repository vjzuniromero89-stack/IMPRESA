...
export function Expenses({expenses,setExpenses,accounts,setAccounts,month,rate,expenseCategories,addExpenseCategory,removeExpenseCategory}:{expenses:Expense[];setExpenses:Setter<Expense>;accounts?:any[];setAccounts?:Setter<any>;month:string;rate:number;expenseCategories?:string[];addExpenseCategory?:(name:string)=>Promise<void>;removeExpenseCategory?:(name:string)=>Promise<void>}){
 ...
 const save=async(e:FormEvent)=>{e.preventDefault();if(busy)return;const original=editing?.enteredAmount??(editing?editing.amount/(editing.currency==='US$'?rate:1):0);const amount=editing&&Number(f.amount)===original&&f.currency===(editing.currency||'C$')?editing.amount:convert(f.amount,f.currency,rate);
  ...
  const entered=Number(f.amount);const available=Number(acct.balance)||0;const restored=editing?.sourceAccountId===acct.id?Number(editing.enteredAmount??(editing.currency==='US$'?editing.amount/rate:editing.amount)):0;
