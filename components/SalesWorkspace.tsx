...
export function Expenses({expenses,setExpenses,accounts,setAccounts,month,rate,expenseCategories,addExpenseCategory,removeExpenseCategory}:{expenses:Expense[];setExpenses:Setter<Expense>;accounts?:any[];setAccounts?:Setter<any>;month:string;rate:number;expenseCategories?:string[];addExpenseCategory?:(name:string)=>Promise<void>;removeExpenseCategory?:(name:string)=>Promise<void>}){
 const categories=(expenseCategories&&expenseCategories.length)?expenseCategories:DEFAULT_EXPENSE_CATEGORIES;
 const empty=()=>({date:today(),category:categories[0]||'Operativo',description:'',amount:'',currency:'C$' as Currency,paymentChannel:'Efectivo' as 'Efectivo'|'BAC',sourceAccountId:''});
 ...
 const matchingAccounts=accountList.filter(a=>a.currency===f.currency&&(f.paymentChannel==='Efectivo'?/efectivo|caja/i.test(a.name):!/efectivo|caja/i.test(a.name)));
 const selectedAccount=matchingAccounts.find(a=>a.id===f.sourceAccountId)||matchingAccounts[0];
 const reset=()=>{setEditing(null);setF(empty())};
 const edit=(x:Expense)=>{setEditing(x);setF({date:x.date,category:x.category,description:x.description,amount:String(x.enteredAmount??x.amount/(x.currency==='US$'?rate:1)),currency:x.currency||'C$',paymentChannel:x.paymentChannel||(/efectivo|caja/i.test(x.sourceAccountName||'')?'Efectivo':'BAC'),sourceAccountId:x.sourceAccountId||''});setMessage('');editor.current?.scrollIntoView({behavior:'smooth'})};
