...
import type {Sale,Expense,Currency,PaymentMethod,InventoryClose,InventoryItem} from '../lib/db';
import {uid} from '../lib/db';
import ManagedSelect from './ManagedSelect';

type Setter<T>=(items:T[])=>void|Promise<void>;
type SalesProps={sales:Sale[];setSales:Setter<Sale>;month:string;rate:number;paymentMethods?:string[];addPaymentMethod?:(name:string)=>Promise<void>;removePaymentMethod?:(name:string)=>Promise<void>;closes?:InventoryClose[];reloadInventory?:()=>void};
...
export function Sales({sales,setSales,month,rate,paymentMethods,addPaymentMethod,removePaymentMethod,closes=[],reloadInventory}:SalesProps){
 const methods=(paymentMethods&&paymentMethods.length)?paymentMethods:DEFAULT_METHODS;
 const empty=()=>({date:today(),client:'',description:'',amount:'',currency:'C$' as Currency,initialPayment:'',paymentMethod:'' as PaymentMethod|'',inventoryItemId:'',quantity:'1'});
 ...
 const selectedProduct=inventory.find(x=>x.id===f.inventoryItemId);
 const chooseProduct=(id:string)=>{const p=inventory.find(x=>x.id===id);setF(prev=>({...prev,inventoryItemId:id,description:p?p.name:prev.description}))};
 ...
 const save=async(e:FormEvent)=>{e.preventDefault();if(busy)return;const initial=editing?paid(editing):convert(f.initialPayment||'0',f.currency,rate);
  const qty=Number(f.quantity)||0;const oldQty=editing?.inventoryItemId===f.inventoryItemId?(editing.quantity||0):0;
  if(selectedProduct&&(qty<=0||qty>selectedProduct.qty+oldQty))return notify(`Cantidad inválida. Existencia disponible: ${selectedProduct.qty+oldQty}.`,true);
  ...
  const sale:Sale={...(editing||{id:uid()}),date:f.date,client:f.client.trim(),description:f.description.trim(),amount:total,currency:f.currency,enteredAmount:editing&&Number(f.amount)===originalEntered&&f.currency===(editing.currency||'C$')?editing.enteredAmount:Number(f.amount),paymentMethod:f.paymentMethod,inventoryItemId:selectedProduct?.id,productCode:selectedProduct?.sku,productName:selectedProduct?.name,productCategory:selectedProduct?.category,talla:selectedProduct?.talla,color:selectedProduct?.color,quantity:selectedProduct?qty:undefined,status:editing&&total===editing.amount?editing.status:status(total,initial),paidAmount:initial,payments:editing?.payments||(initial>0?[{id:uid(),date:f.date,amount:initial,method:f.paymentMethod,note:'Pago inicial'}]:[])};
  setBusy(true);try{await setSales(editing?sales.map(s=>s.id===editing.id?sale:s):[...sales,sale]);notify(editing?'Venta actualizada e inventario ajustado.':'Venta registrada e inventario actualizado.');reloadInventory?.();reset()}catch(err){notify(err instanceof Error?err.message:'No se pudo guardar. Revisa la conexión y vuelve a intentarlo.',true)}finally{setBusy(false)}
 };
 ...
 const remove=async(sale:Sale)=>{if(!confirm('¿Borrar esta venta y su historial de pagos?'))return;setBusy(true);try{await setSales(sales.filter(s=>s.id!==sale.id));if(editing?.id===sale.id)reset();reloadInventory?.();notify('Venta eliminada y existencia restaurada.')}catch{notify('No se pudo borrar la venta.',true)}finally{setBusy(false)}};
 ...
 <Field label="Producto de inventario"><select value={f.inventoryItemId} onChange={e=>chooseProduct(e.target.value)}><option value="">Venta / servicio sin inventario</option>{inventory.map(p=><option key={p.id} value={p.id}>{p.sku||'SIN-COD'} · {p.name} · {p.talla||'—'} · {p.color||'—'} · Stock {p.qty}</option>)}</select></Field>
 {selectedProduct&&<><Field label="Código"><input readOnly value={selectedProduct.sku||'—'}/></Field><Field label="Categoría"><input readOnly value={selectedProduct.category||'—'}/></Field><Field label="Talla"><input readOnly value={selectedProduct.talla||'—'}/></Field><Field label="Color"><input readOnly value={selectedProduct.color||'—'}/></Field><Field label="Cantidad vendida"><input required type="number" min="1" max={selectedProduct.qty+(editing?.inventoryItemId===selectedProduct.id?(editing.quantity||0):0)} step="1" value={f.quantity} onChange={e=>setF({...f,quantity:e.target.value})}/></Field><div className="inventorySaleHint">Existencia actual: <b>{selectedProduct.qty}</b> · Al guardar, esta cantidad se resta automáticamente.</div></>}
 ...
 <h2>Ventas · {month}</h2><GridTable heads={['Fecha','Cliente','Código','Producto / Trabajo','Talla','Color','Cant.','Método','Total','Pagado','Saldo','Estado','Acciones']}>{visible.map(s=><tr key={s.id} className={s.status==='Pendiente'?'saleUnpaid':s.status==='Pago parcial'?'salePending':''}><td>{s.date}</td><td>{s.client}</td><td>{s.productCode||'—'}</td><td>{s.productName||s.description||'—'}</td><td>{s.talla||'—'}</td><td>{s.color||'—'}</td><td>{s.quantity||'—'}</td><td><MethodBadge method={s.paymentMethod}/></td><td>{money(s.amount)}</td><td>{money(paid(s))}</td><td>{money(Math.max(0,s.amount-paid(s)))}</td><td><span className={'statusBadge '+(s.status==='Pagada'?'paid':s.status==='Pendiente'?'pending':'partial')}>{s.status}</span></td><td><div className="actions"><button className="small" disabled={busy} onClick={()=>edit(s)}>Editar</button><button className="small" disabled={busy||paid(s)>=s.amount} onClick={()=>{setPaymentSale(s.id);setPaymentAmount(String(round(s.amount-paid(s))));setPaymentMethod(s.paymentMethod||'Efectivo');setPaymentDate(today());paymentEditor.current?.scrollIntoView({behavior:'smooth'})}}>+ Abono</button><button className="small" onClick={()=>setHistoryId(historyId===s.id?null:s.id)}>Historial</button><button className="dangerSmall" disabled={busy} onClick={()=>remove(s)}>Borrar</button></div></td></tr>)}</GridTable>{!visible.length&&<p className="empty">No hay ventas en este mes.</p>}</div>
...
export function Expenses({expenses,setExpenses,accounts,setAccounts,month,rate,expenseCategories,addExpenseCategory,removeExpenseCategory,expensePaymentMethods,addExpensePaymentMethod,removeExpensePaymentMethod}:{expenses:Expense[];setExpenses:Setter<Expense>;accounts?:any[];setAccounts?:Setter<any>;month:string;rate:number;expenseCategories?:string[];addExpenseCategory?:(name:string)=>Promise<void>;removeExpenseCategory?:(name:string)=>Promise<void>;expensePaymentMethods?:string[];addExpensePaymentMethod?:(name:string)=>Promise<void>;removeExpensePaymentMethod?:(name:string)=>Promise<void>}){
 ...
 const matchingAccounts=accountList.filter(a=>a.currency===f.currency&&(f.paymentChannel==='Efectivo'?/efectivo|caja/i.test(a.name):f.paymentChannel==='Transferencia / BAC'?!/efectivo|caja/i.test(a.name):true));
 const selectedAccount=matchingAccounts.find(a=>a.id===f.sourceAccountId)||matchingAccounts[0];
 const reset=()=>{setEditing(null);setF(empty())};
 ...
 const save=async(e:FormEvent)=>{e.preventDefault();if(busy)return;const original=editing?.enteredAmount??(editing?editing.amount/(editing.currency==='US$'?rate:1):0);const amount=editing&&Number(f.amount)===original&&f.currency===(editing.currency||'C$')?editing.amount:convert(f.amount,f.currency,rate);
  ...
  if(!acct){setError(true);setMessage(`No existe una cuenta ${f.paymentChannel==='Efectivo'?'de efectivo':'BAC/bancaria'} en ${f.currency}. Créala primero en Banco y Efectivo.`);return}
